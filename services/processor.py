...
# Stage codes confirmed from Samsung Galaxy Watch via Health Connect.
# "4" is the primary deep-sleep code the real watch sends.
STAGE_DEEP  = {"4"}          # Samsung deep sleep (only code besides awake sent by real watch)
STAGE_REM   = {"3", "5"}     # REM (standard "3", alternate "5" used by some Wear OS)
STAGE_LIGHT = {"2", "6"}     # Light sleep (standard "2", alternate "6")
STAGE_AWAKE = {"1"}
...
def _extract_fields(raw: dict) -> dict:
    ...
    # Resting HR = 10th percentile of overnight readings
    resting_hr = None
    if overnight_bpms:
        ...
        resting_hr = min(all_bpms)

    avg_hr_overnight = (round(sum(overnight_bpms) / len(overnight_bpms), 1)
                        if overnight_bpms else
                        round(sum(all_bpms) / len(all_bpms), 1) if all_bpms else None)

    avg_hr_day = (round(sum(daytime_bpms) / len(daytime_bpms), 1)
                  if daytime_bpms else avg_hr_overnight)

    hr_std = hr_min_val = hr_max_val = None
    if all_bpms:
        import statistics
        hr_std     = round(statistics.stdev(all_bpms), 2) if len(all_bpms) > 1 else 0.0
        hr_min_val = min(all_bpms)
        hr_max_val = max(all_bpms)
    ...
    return {
        ...
        "avg_heart_rate":       _to_float(avg_hr_overnight),
        "avg_hr_day":           _to_float(avg_hr_day),
        "hr_std":               _to_float(hr_std),
        "hr_min":               hr_min_val,
        "hr_max":               hr_max_val,
        "hrv":                  None,
        "spo2":                 _to_float(spo2),
        "spo2_min":             _to_float(spo2_min),
